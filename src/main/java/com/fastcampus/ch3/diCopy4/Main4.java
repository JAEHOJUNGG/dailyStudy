...
class AppContext{
        ...
        try {
            for(Object bean :map.values()){
                for(Field fld:bean.getClass().getDeclaredFields()){
                    if(fld.getAnnotation(Resource.class)!=null)//Resource 는 byname
    ...
    private void doAutowired() {
        ...
        try {
            for(Object bean :map.values()){
                for(Field fld:bean.getClass().getDeclaredFields()){
                    if(fld.getAnnotation(Autowired.class)!=null)//Autowired 는 bytype
...
public class Main4 {
    public static void main(String[] args) throws  Exception {
        ...
        Door door = (Door) ac.getBean(Door.class);//byType으로 검색
